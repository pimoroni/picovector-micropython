...
void scene_attr(mp_obj_t self_in, qstr attr, mp_obj_t *dest) {
  ...
  switch (attr) {
    case MP_QSTR_surface:
    {
      if (action == GET) { dest[0] = MP_OBJ_FROM_PTR(self->target); return; }
      break;
    }
    case MP_QSTR_meshes:
    {
      if (action == GET) { dest[0] = mp_obj_new_int(self->scene.sub_count); return; }
      break;
    }
    case MP_QSTR_vertices:
    {
      if (action == GET) { dest[0] = mp_obj_new_int(self->scene.vert_count); return; }
      break;
    }
    case MP_QSTR_triangles:
    {
      if (action == GET) { dest[0] = mp_obj_new_int(self->scene.tri_count); return; }
      break;
    }
    case MP_QSTR_mesh_capacity:
    {
      if (action == GET) { dest[0] = mp_obj_new_int(self->scene.sub_cap); return; }
      break;
    }
    case MP_QSTR_vertex_capacity:
    {
      if (action == GET) { dest[0] = mp_obj_new_int(self->scene.vert_cap); return; }
      break;
    }
    case MP_QSTR_triangle_capacity:
    {
      if (action == GET) { dest[0] = mp_obj_new_int(self->scene.tri_cap); return; }
      break;
    }
  }
