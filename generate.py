...
def emit_type_file(t):
    o = Out()
    o(f"// AUTO-GENERATED from {t.stub} by generate.py — do not edit by hand.")
...
def emit_bindings_c(module, types):
    """The module table for one MicroPython module: its type globals, plus (for
    picovector) the __init__ hook, the metrics submodule and the font namespace
    singleton, none of which are types the DSL describes."""
    mod = f"mod{module}"
    is_pv = module == "picovector"
    o = Out()
    o("// AUTO-GENERATED by generate.py — do not edit by hand.")
    o('#include "py/runtime.h"')
    o('#include "types.h"')
    o("")
    if is_pv:
        o(f"extern mp_obj_t {mod}___init__(void);")
        o(f"static MP_DEFINE_CONST_FUN_OBJ_0({mod}___init___obj, {mod}___init__);")
        o("")
        ...
        o(f"extern const mp_obj_module_t {mod}_metrics;")
        o("")
        ...
        o("")
    o(f"static const mp_rom_map_elem_t {mod}_globals_table[] = {{")
    o(f"{{ MP_ROM_QSTR(MP_QSTR___name__), MP_ROM_QSTR(MP_QSTR_{mod}) }},", 1)
    if is_pv:
        o(f"{{ MP_ROM_QSTR(MP_QSTR___init__), MP_ROM_PTR(&{mod}___init___obj) }},", 1)
        o(f"{{ MP_ROM_QSTR(MP_QSTR_metrics), MP_ROM_PTR(&{mod}_metrics) }},", 1)
        o("{ MP_ROM_QSTR(MP_QSTR_font), MP_ROM_PTR(&pv_font_ns_obj) },", 1)
    for t in types:
        o(f"{{ MP_ROM_QSTR(MP_QSTR_{t.name}), MP_ROM_PTR(&{t.mp_type}) }},", 1)
    o("};")
    o(f"static MP_DEFINE_CONST_DICT({mod}_globals, {mod}_globals_table);")
    o("")
    o(f"const mp_obj_module_t {mod} = {{")
    o(".base = { &mp_type_module },", 1)
    o(f".globals = (mp_obj_dict_t *)&{mod}_globals,", 1)
    o("};")
    o("")
    o(f"MP_REGISTER_MODULE(MP_QSTR_{module}, {mod});")
    return str(o)
...
def main():
    ...
    modules = []
    for module in dict.fromkeys(t.module for t in types):
        write_if_changed(os.path.join(args.out, f"{module}_bindings.c"),
                         emit_bindings_c(module, [t for t in types if t.module == module]))
    ...
    print(f"generated {len(written)} type sources + types.h + "
          f"{', '.join(m + '_bindings.c' for m in modules)} + metrics table "
          f"({n_metrics} entries) into {args.out}")
